...
Раз в неделю бот делает `pg_dump` и присылает `.sql` файл в группу уведомлений (`BOT_NOTIFY_CHAT_ID`).  
Для личного бота это удобно: файл лежит в чате, можно скачать на телефон/диск. Лимит Telegram на файл бота — 50 MB (для вашей БД обычно хватает с запасом).

Ручной перенос на другой ПК тоже можно сделать из этого `.sql` файла (или через `pg_dump`/`docker cp`, как удобнее).
